...
class Admininactive
{
    ...
    {
        //var_dump($_POST);
        $sitename = Config::TT()['SITENAME']; // Sitename
        $siteurl = Config::TT()['SITEURL']; // Default site url
        $replyto = Config::TT()['SITEEMAIL']; // The Reply-to email
        // End config
        $record_mail = true; // Set this true or false . If you set this true every time whene you send a mail the time , userid , and the number of mail sent will be recorded
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if ($action == "deluser" && (!empty($_POST["userid"]))) {
                DB::run("DELETE FROM users WHERE id IN (" . implode(", ", $_POST['userid']) . ") ");
            ...
            if ($action == "disable" && (!empty($_POST["userid"]))) {
                ...
                while ($arr = $res->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    DB::run("UPDATE users SET modcomment=" . sqlesc($modcomment) . ", enabled='no' WHERE id=$id ");
                }
