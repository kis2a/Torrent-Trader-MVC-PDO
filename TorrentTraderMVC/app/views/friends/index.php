...
if ($data['friend']->rowCount() == 0) { ?>&nbsp;&nbsp;
    ...
    while ($friend = $data['friend']->fetch(PDO::FETCH_ASSOC)) {
        ...
        <a href='<?php echo  URLROOT ?>/messages/create?id=<?php echo $friend['id'] ?>'><i class="fa fa-comment tticon" title="Send PM"></i></a>&nbsp;
        <a href='<?php echo  URLROOT ?>/friends/delete?id=<?php echo $data['userid'] ?>&type=friend&targetid=<?php echo  $friend['id'] ?>'><i class='fa fa-trash-o tticon-red' title='Remove'></i></a>
...
if ($data['enemy']->rowCount() == 0) { ?>&nbsp;&nbsp;
    ...
    while ($enemy = $data['enemy']->fetch(PDO::FETCH_ASSOC)) {
        ...
        <a href='<?php echo URLROOT ?>/messages/create?id=<?php echo $enemy['id'] ?>'><i class="fa fa-comment tticon" title="Send PM"></i></a>&nbsp;
        <a href='<?php echo URLROOT ?>/friends/delete?id=<?php echo $data['userid']?>&type=friend&targetid=<?php echo  $enemy['id'] ?>'><i class='fa fa-trash-o tticon-red' title='Remove'></i></a>
