...
while ($row = $data['res']->fetch(PDO::FETCH_ASSOC)) {
    ...
    echo "
    <td nowrap='nowrap'><a title=\"" . $row["name"] . "\" href=\"" . URLROOT . "/torrent?id=$id&amp;hit=1\">$dispname</a></td>
    ...
    <td><a href='" . URLROOT . "/comments?type=torrent&id=$row[id]'>" . number_format($row["comments"]) . "</a></td>
