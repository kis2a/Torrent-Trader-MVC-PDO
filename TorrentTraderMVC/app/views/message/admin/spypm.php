...
while ($arr = $data['res']->fetch(PDO::FETCH_ASSOC)) {
    ...
    if ($arr2 = $res2->fetch()) {
        $receiver = "<a href='" . URLROOT . "/users/profile?id=" . $arr["receiver"] . "'><b>" . User::coloredname($arr2["username"]) . "</b></a>";
    } else {
        $receiver = "<i>Deleted</i>";
    }
    $arr3 = DB::run("SELECT username FROM users WHERE id=?", [$arr["sender"]])->fetch();
    $sender = "<a href='" . URLROOT . "/users/profile?id=" . $arr["sender"] . "'><b>" . User::coloredname($arr3["username"]) . "</b></a>";
    if ($arr["sender"] == 0) {
